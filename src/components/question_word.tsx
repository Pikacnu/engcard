...
export default function QuestionWord({
	...
	const currentCards: WithAnswer<CardProps>[] = useMemo(() => {
		if (!cardData || cardData.length === 0 || cardData[0].word === CardWhenEmpty.word) { // Check against translated empty word
			return [
		...
		const questionOptions = shuffle(cardData.filter((_, i) => i !== index))
			.slice(0, 3)
			.map((c) => ({ // Changed card to c to avoid conflict
				...c,
				answer: false,
			}));
    ...
    useEffect(() => {
        ...
        } else if (cardData && cardData.length > 0) { // Handle case where index might be out of bounds temporarily
				...
				{!isCorrect ? (
					<div className='flex flex-col '>
						<div className='text-xl m-4 md:m-8 p-4 bg-gray-100 dark:bg-black dark:bg-opacity-45 rounded-lg flex flex-row flex-wrap items-center *:rounded-lg *:m-1 *:p-2 text-black dark:text-white'>
							<h1 className="font-semibold">{t('components.questionWord.questionLabel')}</h1> {/* Translated */}
							<h1 className='bg-blue-200 dark:bg-blue-600 bg-opacity-80 dark:bg-opacity-40'>{cardData[index]?.word || CardWhenEmpty.word}</h1>
							<h1 className='bg-green-200 dark:bg-green-400 bg-opacity-40 dark:bg-opacity-20 border-2 border-green-300 dark:border-green-500'>
								{
									PartOfSpeechShort[
										cardData[index]?.blocks[0]?.partOfSpeech || PartOfSpeech.Error
									]
								}
							</h1>
						</div>
						<div className='flex flex-col items-center *:w-full space-y-2'>
							{currentCards &&
								currentCards.map((c, i) => ( // Changed card to c
									<button
										key={i}
										...
										onClick={(e) => {
											if (c.answer) {
												...
												e.currentTarget.classList.remove('dark:bg-gray-800', 'bg-white', 'dark:hover:bg-gray-700', 'hover:bg-gray-100');
												e.currentTarget.classList.add('bg-red-500', 'dark:bg-red-600', 'text-white');
											}
										}}
									>
										{c.blocks[0]?.definitions[0]?.definition[0]?.content || CardWhenEmpty.blocks[0].definitions[0].definition[0].content}
									</button>
								))}
						</div>
					</div>
					...
					>
						<Card card={Object.assign(cardData[index] || CardWhenEmpty, { flipped: true })} />
					</div>
				)}
			</div>
			{cardData && cardData.length > 0 && cardData[0].word !== CardWhenEmpty.word && ( // Only show progress if not empty card
