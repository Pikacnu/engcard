...
import type { Blocks, ChatSession, DeckType, OCRProcessType, LangEnum } from "@/type-shared";
...
// Word Cache
export const wordCache = pgTable("word_cache", {
  word: text("word").notNull(),
  ...
  updatedAt: timestamp("updatedAt", { mode: "date" }).defaultNow(),
    ...
    primaryKey({ columns: [t.word, t.targetLang] })
]);


// Saved Decks (Public decks saved by users)
export const savedDecks = pgTable("saved_deck", {
...
export const markedWords = pgTable("marked_word", {
  ...
  uniqueIndex("user_word_deck_unique").on(t.userId, t.word, t.deckId)
]);

// --- Inferred Types ---
export type SettingsSelect = InferSelectModel<typeof settings>;

export type UserSettingsCollection = Omit<SettingsSelect, 'deckActionType' | 'ocrProcessType' | 'targetLang' | 'usingLang'> & {
