...
import { usePathname } from 'next/navigation'; // To get current locale from URL
...
export const LanguageProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const localeSegment = segments[1]; // Assuming URL like /<locale>/...
    if (localeSegment && ['en', 'zh-TW'].includes(localeSegment)) {
      setCurrentLocale(localeSegment);
    } else {
      // Fallback to default if no locale in path or invalid locale
      // This might happen for the root path if not automatically redirected
      setCurrentLocale('en');
    }
  }, [pathname]);
    ...
    const loadTranslations = async () => {
      if (currentLocale) {
        try {
          ...
          console.error(`Could not load translations for ${currentLocale}`, error);
  ...
  return (
    <LanguageContext.Provider value={{ locale: currentLocale, setLocale: handleSetLocale, translations: loadedTranslations, t: translate }}>
      {children}
